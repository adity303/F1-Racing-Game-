...
texture = pygame.image.load(os.getcwd()+'\\texture.png')
...
def things_dodged(count, high_score, thing_speed):
	...
	score = font.render("Dodged: "+str(count), True, lightblue)
	highscore = font.render("High Score: "+str(high_score), True, lightblue)
	speed = font.render("Speed: "+str(thing_speed)+"Km/h", True, lightblue)
...
def things(thing, thingy):
	gameDisplay.blit(obstacle_Img,(thing,thingy))
...
def title_msg(shift_x, shift_y, color):
	...
	TextSurf, TextRect = text_objects("F1 Racing Game", largeText, color)
...
def crash():
	...
	while True:
		play = button("Play Again", button_start_x, new_game_y, button_width, button_height, lightblue, green)
	...
	while intro:
		for event in pygame.event.get():
			if event.type == pygame.QUIT or quit_game == 1 or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
				pygame.quit()
				quit()
		play = button("New game", button_start_x, new_game_y, button_width, button_height, lightblue, blue)
		quit_game = button("Quit", button_start_x, quit_y, button_width, button_height, redLight, red)
